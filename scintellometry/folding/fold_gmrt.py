...
def fold(file1, file2, dtype, samplerate, fbottom, fband, nblock,
    ...
    half_data_bug : bool
        whether streams only alternating seconds of data
    paired_samples_bug : bool
        whether every even sample is sum of preceding sample and itself
    ...
    with open(file1, 'rb', recsize) as fh1, open(file2, 'rb', recsize) as fh2:
        ...
        if np.issubdtype(dtype, np.int):
            ...
            foldspec = np.zeros((nblock, ngate))
            icount = np.zeros((nblock, ngate))
        ...
        for j in xrange(nt):
            ...
            if not half_data_bug or j % 16 < 8*1:
                ...
                try:
                    ...
                    if two_streams:
                        raw = np.hstack([np.fromfile(fh, dtype=dtype,
                                                     count=recsize)
                                         for fh in (fh1, fh2)
                                         ]).reshape(-1,nblock,2)
                    else:
                        raw = np.fromfile(fh1, dtype=dtype,
                                          count=recsize*2).reshape(-1,nblock,2)
                except:
                    break
